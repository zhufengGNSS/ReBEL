...
for i=1:NOV,
    ...
    A = InferenceDS.linearize( InferenceDS, state, pNoise.mu, [], UU1, [], 'A');
    G = InferenceDS.linearize( InferenceDS, state, pNoise.mu, [], UU1, [], 'G');

    xh_(:,i) = InferenceDS.ffun( InferenceDS, state, pNoise.mu, UU1);
    Px_      = A*Pstate*A' + G*pNoise.cov*G';

    %--- MEASUREMENT UPDATE
    C = InferenceDS.linearize( InferenceDS, xh_(:,i), [], oNoise.mu, [], UU2, 'C');
    H = InferenceDS.linearize( InferenceDS, xh_(:,i), [], oNoise.mu, [], UU2, 'H');
    ...
    case 'parameter'  %--- parameter estimation
        ...
        otherwise
            error(' [kf]unknown process noise adaptation method!');
        end
